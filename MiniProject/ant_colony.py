...
class ant_colony:
	class ant(Thread):
		def __init__(self, init_location, possible_locations, pheromone_map, distance_callback, path_callback, alpha, beta, gamma, D, first_pass=False):
			"""
		...
		def _traverse(self, start, end):
			...
			if not is_valid:
				...
				self._update_route(start, end)
				self.location = end
		
		def _update_route(self, start=None, new=None):
			"""
			...
			path = self.path_callback(start, new)
			self.route[self.k].extend(path[1:])
			self.possible_locations = [item for item in self.possible_locations if item not in path]
	...
	def __init__(self, nodes, distance_callback, path_callback, start=None, ant_count=50, alpha=2.0, beta=5.0, gamma = 9.0, D = 100, pheromone_evaporation_coefficient=.20, theta=0.2, pheromone_constant=1.0, iterations=80):
		"""
		...
		if not callable(path_callback):
			raise TypeError("path_callback is not callable, should be method")
	...
	def _init_ants(self, start):
		...
		if self.first_pass:
			return [self.ant(start, list(self.nodes), self.pheromone_map, self._get_distance, self.path_callback,
				self.alpha, self.beta, self.gamma, self.D, first_pass=True) for x in range(self.ant_count)]
		#else, just reset them to use on another pass
		for ant in self.ants:
			ant.__init__(start, list(self.nodes), self.pheromone_map, self._get_distance, self.path_callback, self.alpha, self.beta, self.gamma, self.D)
	...
	def _update_pheromone_map(self, ants):
		...
		it_distance = 0
		for ant in ants:
			it_distance += sum(ant.get_distance_traveled())

		L_avg = float(it_distance) / self.ant_count
		...
		for (ant, L) in good_ants:
			...
			for path in route:
				for i in range(0, len(path) - 1):
					...
					new_pheromone_value = self.pheromone_constant * (1.0 / float(self.best_L)) + (3.0 * (3 - meu)) / float(N * L)
	...
	def mainloop(self):
		...
		for it in range(self.iterations):
			...
			print('Iteration', it + 1, 'K ', self.best_K)
		...
		return ret, self.best_distance, k